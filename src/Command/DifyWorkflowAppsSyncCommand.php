...
#[AsCommand(
    name: self::NAME,
    description: '同步所有Workflow类型的Dify应用'
)]
#[WithMonologChannel(channel: 'dify_console_api')]
class DifyWorkflowAppsSyncCommand extends Command
{
    public const NAME = 'dify:sync:workflow-apps';
    private const APP_TYPE = 'workflow';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Dify Workflow应用同步');
        $io->info("同步范围: {$scope}");
        $io->info('应用类型: Workflow');
    ...
    private function executeAsync(SymfonyStyle $io, ?int $instanceId, ?int $accountId): int
    {
        $message = new DifySyncMessage(
            ...
            metadata: [
                'request_id' => uniqid('workflow_sync_', true),
                'initiated_at' => new \DateTimeImmutable(),
                'source' => 'workflow_sync_command',
            ]
        );

        $this->messageBus->dispatch($message);

        $io->success('Workflow应用同步任务已发送到消息队列');
    ...
    private function executeSync(SymfonyStyle $io, ?int $instanceId, ?int $accountId): int
    {
        ...
        $progressBar->setMessage('准备同步Workflow应用...');
        $progressBar->start();

        try {
            $progressBar->setMessage('同步Workflow应用数据...');
            $progressBar->advance();
